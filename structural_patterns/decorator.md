...
- 继承:类的继承, 利用继承设计子类行为,是在编译时静态决定的,且所有子类都会继承到相同行为.
- 组合:对象的组合利用组合方式扩展对象行为,可在运行时动态进行扩展.


模式定义
--------------------
装饰模式(Decorator Pattern) ：动态地给一个对象增加一些额外的职责(Responsibility).(将对象拆分成组件和装饰者,通过利用组合和委托的方式实现对象的行为扩展)
